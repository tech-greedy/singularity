...
program.option('-c, --client <addresses...>', 'List of client addresses to filter the deal proposals', '*')
  ...
  .addOption(new Option('-j, --download-threads <download_threads>', 'The number of concurrent threads for downloading')
    .argParser(parseInt)
    .default(8))
  ...
  .addOption(new Option('-t, --interval <interval>',
    'The interval in seconds between handling applicable deals, including file downloads. ' +
    ...
    .argParser(parseInt)
    .default(120))
  .addOption(new Option('-m, --concurrency <concurrency>',
  ...
  .addOption(new Option('-i, --import-concurrency <import_concurrency>',
    'This sets an upper limit of concurrent imports when the value from --import-interval is less than the actual time the storage provider spends to import a deal.')
    .argParser(parseInt).default(1))
